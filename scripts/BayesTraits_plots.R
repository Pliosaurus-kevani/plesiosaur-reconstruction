...
rate.plot.all <- 
  ...
  scale_color_continuous(name = expression(paste(log[10],"(rates)")), oob=scales::squish, low = "blue", high = "yellow") +
...
rate.plot.strict <- 
  ...
  scale_color_continuous(name = "rates", oob=scales::squish, low = "blue", high = "yellow") +
  geom_tiplab(size = 2.5, color = "black", fontface="italic") +
  #add this line if you need to manually check whether the rates are projected to the correct branches
  #geom_text2(aes(label=(10^(evol_rates))),hjust=-.3,color="red", size = 1.5) +
...
ggsave(filename = "Figure4.pdf", figure, width = 13, height = 13)
